...
- `hypothesis_agent`, `critic_agent`, and `knowledge_agent_pool` are the role agents. They have explicit goals, tools, loop steps, and traces.
...
The current integration uses Modal functions for evidence retrieval. The next natural extension is to promote `hypothesis_agent`, `critic_agent`, and `knowledge_agent_pool` into OpenAI Agents SDK agents with Modal sandbox sessions for longer-lived legal research experiments.
